/**
 * @file    Aspanformer.cpp
 *
 *
 */

#include "ASpanFormer.hpp"
#include "Utility.hpp"
#include <utility>
...
processOneImagePair(const Ort::ASpanFormer& loftrOsh, const cv::Mat& queryImg, const cv::Mat& refImg, float* queryData,
...
int main(int argc, char* argv[])
{
    ...
    std::vector<float> queryData(Ort::ASpanFormer::IMG_CHANNEL * Ort::ASpanFormer::IMG_H * Ort::ASpanFormer::IMG_W);
    std::vector<float> refData(Ort::ASpanFormer::IMG_CHANNEL * Ort::ASpanFormer::IMG_H * Ort::ASpanFormer::IMG_W);

    Ort::ASpanFormer osh(
        ONNX_MODEL_PATH, 0,
        std::vector<std::vector<int64_t>>{{1, Ort::ASpanFormer::IMG_CHANNEL, Ort::ASpanFormer::IMG_H, Ort::ASpanFormer::IMG_W},
                                          {1, Ort::ASpanFormer::IMG_CHANNEL, Ort::ASpanFormer::IMG_H, Ort::ASpanFormer::IMG_W}});
    ...
    cv::imwrite("aspanformer.jpg", matchesImage);
    cv::imshow("aspanformer", matchesImage);
    cv::waitKey();
                    ...
                    float* refData, float confidenceThresh)
{
    ...
    cv::resize(queryImg, scaledQueryImg, cv::Size(Ort::ASpanFormer::IMG_W, Ort::ASpanFormer::IMG_H), 0, 0, cv::INTER_CUBIC);
    cv::resize(refImg, scaledRefImg, cv::Size(Ort::ASpanFormer::IMG_W, Ort::ASpanFormer::IMG_H), 0, 0, cv::INTER_CUBIC);

    loftrOsh.preprocess(queryData, scaledQueryImg.data, Ort::ASpanFormer::IMG_W, Ort::ASpanFormer::IMG_H, Ort::ASpanFormer::IMG_CHANNEL);
    loftrOsh.preprocess(refData, scaledRefImg.data, Ort::ASpanFormer::IMG_W, Ort::ASpanFormer::IMG_H, Ort::ASpanFormer::IMG_CHANNEL);
    ...
    for (int i = 0; i < numKeyPoints; ++i) {
        ...
        queryKpt.pt.x = queryX * origQueryW / Ort::ASpanFormer::IMG_W;
        queryKpt.pt.y = queryY * origQueryH / Ort::ASpanFormer::IMG_H;

        refKpt.pt.x = refX * origRefW / Ort::ASpanFormer::IMG_W;
        refKpt.pt.y = refY * origRefH / Ort::ASpanFormer::IMG_H;
